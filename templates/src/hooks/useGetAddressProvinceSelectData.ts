...
export function useGetAddressProvinceInputData(type: string, debounceApiCall: () => void): IAddressProvinceHookProps{
    ...
    const provinceOptions = map(province, province => ({ value: province.iso_code, name: province.name }));
    ...
    return {placeholder, label, id, name , value, showProvince, provinceOptions, handleChange, errorMessage};
}
