...
import {useDispatchAutocompleteBillingData} from 'src/hooks';
...
describe('Testing hook useDispatchAutocompleteBillingData', () => {
    beforeEach(() => {
        jest.resetAllMocks();
    });
    ...
    test.each(autocompleteDataSet)(
        ...
        ({numCalls, autocompleteData}) => {
            const {result} = renderHook(() => useDispatchAutocompleteBillingData());
