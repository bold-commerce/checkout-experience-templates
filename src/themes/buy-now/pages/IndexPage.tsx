...
import { CartItems, CondensedSection, CondensedShipping } from 'src/components';
...
function IndexPage(props: IBuyNowContainerPageProps, ref: ForwardedRef<HTMLDivElement>): React.ReactElement {
    ...
    const billing = getTerm('same_as_shipping', Constants.PAYMENT_INFO);
    ...
    return (
        <div ref={ref} className={ClassNames('buy-now', props.show ? 'buy-now--open' : 'buy-now--closed')}>
            <div className='buy-now__header'>{websiteName}</div>
            ...
            <CondensedSection {...{ className: ClassNames(['buy-now__shipping', 'buy-now__section']), navigationHeadingProps: { text: shippingHeadingText, navigation: () => props.navigateTo('/shipping') } }} >
                <CondensedShipping address={ address } includeMethod /> 
            </CondensedSection>        
            <CondensedSection {...{ className: ClassNames(['but-now__payment', 'buy-now__section']), navigationHeadingProps: { text: paymentHeadingText } }} >
